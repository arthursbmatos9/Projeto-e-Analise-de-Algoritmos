...
void testarEscalabilidade() {
    ...
    for (int n : tamanhos) {
        ...
        std::cout << "Distância TED: " << distancia << std::endl;
...
void criarArvoresDiferentesCom10Nos(Tree<int>& tree1, Tree<int>& tree2) {
    tree1 = Tree<int>(0);
    for (int i = 1; i < 10; ++i) {
        int pai = (i - 1) / 2;
        tree1.adicionarFilho(pai, i);
    }

    tree2 = Tree<int>(0);
    for (int i = 1; i < 10; ++i) {
        tree2.adicionarFilho(i - 1, i);
    }
}
...
void testarArvores_4x5() {
    ...
    std::cout << "\n=== TESTE 10: ÁRVORES COM 4 E 5 VÉRTICES ===" << std::endl;
...
void testarArvores_6x7() {
    std::cout << "\n=== TESTE 11: ÁRVORES COM 6 E 7 VÉRTICES ===" << std::endl;
    ...
    try {
        double distancia = zhangShashaDistance(tree3, tree4);
        std::cout << "Distância: " << distancia << std::endl;
    } catch (const std::exception& e) {
        std::cout << "ERRO: " << e.what() << std::endl;
    }
